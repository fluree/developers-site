...
a set of subjects that have a `"schema:name"` equal to `"Alice Smith"` is bound
...
the set `"?s"` would include all nodes that have _both_ a `"schema:givenName"`
of `"Alice"` _and_ a `"schema:familyName"` of `"Smith"`.
...
```json snippet=reference/querying/where-clauses/union
```
...
```json snippet=reference/querying/where-clauses/bind
```
...
```json snippet=reference/querying/where-clauses/optional focus=5
```
...
>
> 1. `["optional", { "@id": "?s", "ex:bestFriend": "?friend"}, { "@id": "?s", "schema:name": "?name"}]`
> 2. `["optional", { "@id": "?s", "ex:bestFriend": "?friend", "schema:name": "?name"}]`
>
> In #1, an entity could have a value on **either** `ex:bestFriend` or `schema:name`, and that value would then be bound to `?friend` or `?name`, respectively (the other would be `null`).
>
> In #2, if an entity had **only a value on either `ex:bestFriend` or `schema:name`**, then **BOTH `?friend` and `?name`** would be bound to `null`.
