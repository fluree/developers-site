import Announcement from '../../src/components/mdx_partials/_Announcement.mdx';
...
There is a built-in user interface which we call the **AdminUI**. You can access the AdminUI by navigating to `localhost:[port]`, once you have an instance of Fluree running on your machine. If you did not change the port or IP address, it will be on [`localhost:8090`](http://localhost:8090/).
