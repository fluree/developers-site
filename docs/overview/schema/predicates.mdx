...
Below are the built-in predicates for the `_predicate` collection. As mentioned in the [collection section](/overview/schema/collections.mdx), you can add additional predicates if you wish.
...
| `_predicate/deprecated`         | `boolean` | (optional) True if this v is deprecated. Reads and writes are still allowed, but might give warnings in the API.                                                                                                                                                                                                                                                                                                                                                                 |
...
| `_predicate/retractDuplicates`           | `boolean` | (optional) If false (default), when a transaction creates a new flake that already exists it does not update the existing data. When true, it will always force a retraction/insertion.                                                                                                                                                                                                                                                                                    |
...
If you would like a `_predicate/spec` or `txSpec`, which limits the allowed values for a predicate, see the [function](/overview/schema/smartfunctions.mdx) section.
